    ...
    try {
      ...
      if (checkSyntax) {
        ...
        for (const line of lines) {
          if (line.includes(":")) {
            if (/^syntax:.?glob/.test(line)) allow = true;
            if (/^syntax:.?regex/.test(line)) allow = false;
            if (line.startsWith("glob:") || line.startsWith("path:")) {
              result.push(line.slice(line.indexOf(":") + 1));
            }
          } else {
